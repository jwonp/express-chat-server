export function makeClient(token: string, login: string) {
  const WebSocketClient = require("websocket").client;
  const client = new WebSocketClient();
  const channel = `#${login}`; // Replace with your channel.
  const password = `oauth:${token}`;

  const account = login; // Replace with the account the bot runs as
              ...
              case "PRIVMSG":
                ...
                if ("move" === parsedMessage.command.botCommand) {
                  ...
                  let updateInterval = parsedMessage.command.botCommandParams
                    ? parseInt(parsedMessage.command.botCommandParams) *
                      1000 *
                      60
                    : defaultMoveInterval;
